...
#pragma omp parallel for num_threads(Params.thread_num_) schedule(dynamic) default(none)
        ...
        for (size_t i = 0; i < num_; i++) {
            ...
            for (unsigned j = 0; j < nn_size_; j++) {
                ...
                IDType id = cur[j];
                if (id == i) continue;
                dist_op_.calculate(data_modal1_ + (size_t)i * dim1_, data_modal1_ + (size_t)id * dim1_,
                                   dim1_, dim1_,
                                   data_modal2_ + (size_t)i * dim2_, data_modal2_ + (size_t)id * dim2_,
                                   dim2_, dim2_, dist);
...
#pragma omp parallel for num_threads(Params.thread_num_) schedule(dynamic) shared(s, g) default(none)
        ...
        for (unsigned i = 0; i < s.size(); i++) {
            ...
            for (unsigned j = 0; j < num_; j++) {
                DistResType dist = 0;
                dist_op_.calculate(data_modal1_ + (size_t)s[i] * dim1_, data_modal1_ + (size_t)j * dim1_,
                                   dim1_, dim1_,
                                   data_modal2_ + (size_t)s[i] * dim2_, data_modal2_ + (size_t)j * dim2_,
                                   dim2_, dim2_, dist);
...
#pragma omp atomic
    ...
    CStatus bi_insert(IDType a, IDType b) {
        DistResType dist = 0;
        dist_op_.calculate(data_modal1_ + (size_t)a * dim1_, data_modal1_ + (size_t)b * dim1_,
                           dim1_, dim1_,
                           data_modal2_ + (size_t)a * dim2_, data_modal2_ + (size_t)b * dim2_,
                           dim2_, dim2_,dist);
        insert(a, b, dist);
        insert(b, a, dist);
        return CStatus();
