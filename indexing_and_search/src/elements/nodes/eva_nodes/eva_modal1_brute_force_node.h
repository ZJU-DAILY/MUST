...
public:
    CStatus init() override {
        ...
        printf("[PARAM] modal 1 query vector num: %d\n", model_param->search_meta_modal1_.num);
    ...
    CStatus run() override {
        ...
        s_param->results_modal1.resize(query_num_);
...
#pragma omp parallel for num_threads(Params.thread_num_) schedule(dynamic) \
        ...
        for (IDType i = 0; i < query_num_; i++) {
            ...
            for (IDType j = 0; j < num_; j++) {
                ...
                DistResType dist = 0;
                dist_op_.calc_vec1(m_param->search_meta_modal1_.data + (i * dim1_),
