...
public:
    CStatus init() override {
        ...
        CStatus status = model_param->train_meta_modal2_.load(Params.GA_ALG_BASE_MODAL2_PATH_,
                                                       Params.is_norm_modal2_);

        printf("[PATH] modal 2 vector path: %s\n", Params.GA_ALG_BASE_MODAL2_PATH_);
        printf("[PARAM] modal 2 vector num: %d\n", model_param->train_meta_modal2_.num);
        ...
        printf("[PARAM] modal 2 query vector num: %d\n", model_param->search_meta_modal2_.num);
        printf("[PARAM] modal 2 query vector dim: %d\n", model_param->search_meta_modal2_.dim);
        num_ = model_param->train_meta_modal2_.num;
        query_num_ = model_param->search_meta_modal2_.num;
...
#pragma omp parallel for num_threads(Params.thread_num_) schedule(dynamic) \
        ...
        for (IDType i = 0; i < query_num_; i++) {
            ...
            for (IDType j = 0; j < num_; j++) {
                ...
                DistResType dist = 0;
                dist_op_.calc_vec2(m_param->search_meta_modal2_.data + (i * dim2_),
